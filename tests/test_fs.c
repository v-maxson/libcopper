...
#define TEST_DEEP \
	TEST_DIR CPR_FS_SEP_STR "a" CPR_FS_SEP_STR "b" CPR_FS_SEP_STR "c"
...
{
	TEST_ASSERT_TRUE(cpr_write_file_all(TEST_FILE, data, len));
}
...
void test_dirname_with_parent(void)
{
	...
	TEST_ASSERT_TRUE(cpr_path_dirname(buf, sizeof buf, "/a/b/c.txt"));
...
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_path_dirname(buf, sizeof buf, "file.txt"));
...
void test_dirname_null(void)
{
	char buf[CPR_FS_PATH_MAX];
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_path_dirname(buf, sizeof buf, NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_path_dirname(NULL, sizeof buf, "/a/b"));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_path_join(buf, sizeof buf, "base", "part"));
...
void test_join_abs_part_replaces_base(void)
{
	char buf[CPR_FS_PATH_MAX];
#if defined(CPR_PLATFORM_WINDOWS)
	TEST_ASSERT_TRUE(cpr_path_join(buf, sizeof buf, "base", "C:\\abs"));
	TEST_ASSERT_EQUAL_STRING("C:\\abs", buf);
#else
	TEST_ASSERT_TRUE(cpr_path_join(buf, sizeof buf, "base", "/abs"));
...
void test_join_null(void)
{
	char buf[CPR_FS_PATH_MAX];
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_path_join(buf, sizeof buf, NULL, "part"));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_path_join(buf, sizeof buf, "base", NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_path_join(NULL, sizeof buf, "base", "part"));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_normalize_path(buf, sizeof buf, "a/./b"));
...
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_normalize_path(buf, sizeof buf, "a/b/../c"));
...
void test_normalize_collapses_seps(void)
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_normalize_path(buf, sizeof buf, "a//b///c"));
	TEST_ASSERT_EQUAL_STRING("a" CPR_FS_SEP_STR "b" CPR_FS_SEP_STR "c",
				 buf);
}
...
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_normalize_path(buf, sizeof buf, "."));
	TEST_ASSERT_EQUAL_STRING(".", buf);
}
...
void test_normalize_null(void)
{
	char buf[CPR_FS_PATH_MAX];
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_normalize_path(buf, sizeof buf, NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_normalize_path(NULL, sizeof buf, "a/b"));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	char buf[CPR_FS_PATH_MAX];
	TEST_ASSERT_TRUE(cpr_cwd(buf, sizeof buf));
...
void test_cwd_null(void)
{
	char buf[CPR_FS_PATH_MAX];
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_cwd(NULL, sizeof buf));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_cwd(buf, 0));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	CprFsStat st;
	TEST_ASSERT_TRUE(cpr_fs_stat(TEST_DIR, &st));
...
void test_stat_nonexistent(void)
{
	CprFsStat st;
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_stat(TEST_DIR CPR_FS_SEP_STR "no_such", &st));
...
void test_stat_null(void)
{
	CprFsStat st;
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_stat(NULL, &st));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_stat(TEST_DIR, NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	TEST_ASSERT_TRUE(cpr_mkdir(TEST_SUBDIR));
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_mkdir(TEST_DIR));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_mkdir(TEST_DIR CPR_FS_SEP_STR
				    "no_parent" CPR_FS_SEP_STR "child"));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_mkdir(NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}

void test_mkdir_all_deep(void)
{
	TEST_ASSERT_TRUE(cpr_mkdir_all(TEST_DEEP));
...
void test_mkdir_all_already_exists(void)
{
	...
	TEST_ASSERT_TRUE(cpr_mkdir_all(TEST_DIR));
}

void test_mkdir_all_null(void)
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_mkdir_all(NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
void test_remove_file_success(void)
{
	...
	TEST_ASSERT_TRUE(cpr_remove_file(TEST_FILE));
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_remove_file(TEST_DIR));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_remove_file(TEST_FILE));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_remove_file(NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	TEST_ASSERT_TRUE(cpr_mkdir(TEST_SUBDIR));
	TEST_ASSERT_TRUE(cpr_remove_dir(TEST_SUBDIR, false));
...
void test_remove_dir_nonempty_no_force(void)
{
	write_test_file("x", 1);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_remove_dir(TEST_DIR, false));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
	TEST_ASSERT_TRUE(cpr_path_exists(TEST_DIR));
}
...
{
	write_test_file("x", 1);
	TEST_ASSERT_TRUE(cpr_remove_dir(TEST_DIR, true));
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_remove_dir(NULL, false));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	write_test_file("hello", 5);
	TEST_ASSERT_TRUE(cpr_fs_rename(TEST_FILE, TEST_RENAMED));
	TEST_ASSERT_FALSE(cpr_path_exists(TEST_FILE));
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_rename(TEST_FILE, TEST_RENAMED));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_rename(NULL, TEST_RENAMED));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_rename(TEST_FILE, NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
void test_copy_file(void)
{
	...
	TEST_ASSERT_TRUE(cpr_fs_copy(TEST_FILE, TEST_COPY));
...
{
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_copy(NULL, TEST_COPY));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_fs_copy(TEST_FILE, NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_INVALID, cpr_get_error().code);
}
...
{
	cpr_clear_error();
	CprFile *f = cpr_open_file(TEST_FILE, CPR_FILE_READ);
	TEST_ASSERT_NULL(f);
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	CprFile *f = cpr_open_file(TEST_FILE, CPR_FILE_WRITE);
...
void test_open_append_preserves_content(void)
{
	...
	CprFile *f = cpr_open_file(TEST_FILE, CPR_FILE_APPEND);
	TEST_ASSERT_NOT_NULL(f);
	cpr_write_file(f, "_second", 7);
	cpr_close_file(f);
...
{
	TEST_ASSERT_NULL(cpr_open_file(NULL, CPR_FILE_READ));
}
...
void test_write_and_read_back(void)
{
	...
	CprFile *w = cpr_open_file(TEST_FILE, CPR_FILE_WRITE);
	TEST_ASSERT_NOT_NULL(w);
	TEST_ASSERT_EQUAL_INT((int)len, (int)cpr_write_file(w, data, len));
	cpr_close_file(w);

	CprFile *r = cpr_open_file(TEST_FILE, CPR_FILE_READ);
	TEST_ASSERT_NOT_NULL(r);
	char buf[64] = { 0 };
	TEST_ASSERT_EQUAL_INT((int)len, (int)cpr_read_file(r, buf, sizeof buf));
...
void test_read_partial(void)
{
	...
	char buf[4] = { 0 };
	TEST_ASSERT_EQUAL_INT(3, (int)cpr_read_file(f, buf, 3));
...
void test_seek_and_tell(void)
{
	...
	TEST_ASSERT_EQUAL_INT(0, (int)cpr_tell_file(f));

	TEST_ASSERT_TRUE(cpr_seek_file(f, 5, CPR_SEEK_START));
	TEST_ASSERT_EQUAL_INT(5, (int)cpr_tell_file(f));

	TEST_ASSERT_TRUE(cpr_seek_file(f, -2, CPR_SEEK_END));
	TEST_ASSERT_EQUAL_INT(8, (int)cpr_tell_file(f));

	TEST_ASSERT_TRUE(cpr_seek_file(f, 1, CPR_SEEK_CURRENT));
	TEST_ASSERT_EQUAL_INT(9, (int)cpr_tell_file(f));
...
void test_file_size(void)
{
	...
	CprFile *f = cpr_open_file(TEST_FILE, CPR_FILE_READ);
	TEST_ASSERT_NOT_NULL(f);
	TEST_ASSERT_EQUAL_INT(5, (int)cpr_file_size(f));
...
void test_file_size_empty(void)
{
	CprFile *f = cpr_open_file(TEST_FILE, CPR_FILE_WRITE);
	TEST_ASSERT_NOT_NULL(f);
	TEST_ASSERT_EQUAL_INT(0, (int)cpr_file_size(f));
	cpr_close_file(f);
}
...
void test_flush(void)
{
	CprFile *f = cpr_open_file(TEST_FILE, CPR_FILE_WRITE);
	TEST_ASSERT_NOT_NULL(f);
	cpr_write_file(f, "data", 4);
	TEST_ASSERT_TRUE(cpr_flush_file(f));
	cpr_close_file(f);
}
...
void test_eof_set_after_full_read(void)
{
	...
	cpr_read_file(f, buf, sizeof buf);
...
void test_read_file_all_success(void)
{
	...
	char buf[32] = { 0 };
	size_t n = 0;
	TEST_ASSERT_TRUE(cpr_read_file_all(TEST_FILE, buf, sizeof buf, &n));
	TEST_ASSERT_EQUAL_INT(11, (int)n);
...
void test_read_file_all_null_out_size(void)
{
	write_test_file("hello", 5);
	char buf[32] = { 0 };
	TEST_ASSERT_TRUE(cpr_read_file_all(TEST_FILE, buf, sizeof buf, NULL));
...
void test_read_file_all_overflow(void)
{
	...
	char buf[4];
	cpr_clear_error();
	TEST_ASSERT_FALSE(cpr_read_file_all(TEST_FILE, buf, sizeof buf, NULL));
	TEST_ASSERT_EQUAL_INT(CPR_ERR_OVERFLOW, cpr_get_error().code);
}
...
{
	TEST_ASSERT_TRUE(cpr_write_file_all(TEST_FILE, "content", 7));
	CprFsStat st;
	cpr_fs_stat(TEST_FILE, &st);
...
void test_write_file_all_overwrites(void)
{
	...
	TEST_ASSERT_TRUE(cpr_write_file_all(TEST_FILE, "short", 5));
	CprFsStat st;
	cpr_fs_stat(TEST_FILE, &st);
	TEST_ASSERT_EQUAL_INT(5, (int)st.size);
}
...
{
	CprDirIterator *it = cpr_open_dir(TEST_DIR);
...
{
	cpr_clear_error();
	CprDirIterator *it = cpr_open_dir(TEST_DIR CPR_FS_SEP_STR "no_such");
	TEST_ASSERT_NULL(it);
	TEST_ASSERT_EQUAL_INT(CPR_ERR_IO, cpr_get_error().code);
}
...
{
	TEST_ASSERT_NULL(cpr_open_dir(NULL));
}
